...
import { getAiTravelResponse } from '../../services/geminiService';
...
const AiAssistant: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    { sender: 'ai', text: 'Hello! How can I help? नमस्ते! मैं आपकी कैसे मदद कर सकता हूँ? வணக்கம்! நான் எப்படி உதவ முடியும்?' }
  ]);
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-start gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
              ...
              <div className={`max-w-xs md:max-w-md p-3 rounded-xl ${msg.sender === 'user' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-700 border dark:bg-slate-700 dark:border-slate-600 dark:text-slate-200'}`}>
      ...
      <div className="flex">
        <input
          ...
          placeholder="Ask 'अगली बस कब है?' or 'அடுத்த பஸ் எப்போ?'"
          className="flex-grow bg-white border-slate-300 rounded-l-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-100"
          disabled={isLoading}
        />
        <button
          onClick={handleSend}
          disabled={isLoading}
          className="bg-indigo-600 text-white px-4 py-2 rounded-r-lg font-semibold hover:bg-indigo-700 disabled:bg-indigo-400"
        >
          Send
        </button>
      </div>
